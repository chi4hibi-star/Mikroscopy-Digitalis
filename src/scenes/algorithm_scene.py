...
class AlgorithmScene():
    def __init__(self, screen, settings, switch_scene_callback):
        ...
        self.algorithm_definitions = self.load_algorithm_definitions()
    ...
    def update_layout(self,window_width,window_height):
        ...
        self.node_viewer.update_layout((window_width,window_height))
        self.canvas.update_layout((window_width,window_height))
        self.parameter_panel.update_layout((window_width,window_height))
        self.algorithm_viewer.update_layout((window_width,window_height))
        return
    ...
    def draw(self, screen):
        ...
        self._draw_dragging(*self.node_viewer.get_dragging_node(),screen)
        self._draw_dragging(*self.algorithm_viewer.get_dragging_node(),screen)
        return
    
    def _draw_dragging(self, dragging_node, mouse_pos, screen):
        if dragging_node:
    ...
    def setup_algorithm_viewer(self):
        ...
        for category in self.algorithm_definitions.get('categories', []):
            ...
            for algorithm in category.get('algorithms', []):
                ...
                color = tuple(algorithm.get('color', [100, 150, 200]))
    ...
    def setup_working_directory(self):
        self.working_dir = Path.cwd() / "pipelines"
    ...
    def load_node_definitions(self):
        ...
        try:
            json_path = Path("nodes_definition.json")
            if not json_path.exists():
                print("Warning: nodes_definition.json not found, using empty definitions")
                return {"categories": []}
    ...
    def load_algorithm_definitions(self):
        try:
            pipeline_dir = Path.cwd() / "pipelines"
            ...
            for pipeline_file in pipeline_files:
                try:
                    ...
                    # Create an algorithm entry from the pipeline
                    algorithm = {
                        "name": pipeline_file.stem,                                 # Use filename without extension
                        "description": f"Saved pipeline: {pipeline_file.name}",
                        "color": [150, 100, 200],
                        #"pipeline_file": str(pipeline_file),                       # when description is added when saving a pipeline, the pipeline file description can be added back in
    ...
    def _update_parameter_panel_for_node(self, node):
    ...
    def _get_parameter_definitions(self, node_name):
    ...
    def _add_algorithm_node(self, algorithm_template, mouse_pos):
        """Add a single algorithm node that encapsulates the pipeline"""
        try:
            ...
            if not algorithm_data:
                ...
                return
            # Create the algorithm node using the canvas method
    ...
    def _serialize_pipeline(self):
        ...
        for idx, node in enumerate(self.canvas.nodes):
            ...
            if node.node_type == "algorithm":
                ...
                print(f"Serializing algorithm node: {node.name}")
            print(f"  Has pipeline_data: {node_data['pipeline_data'] is not None}")
    ...
    def _deserialize_pipeline(self, pipeline_data):
        """Load a pipeline from JSON data"""
        nodes_to_remove = [n for n in self.canvas.nodes if n.node_type in ["process", "algorithm"]]
        for node in nodes_to_remove:
            self.canvas.remove_node(node)
        node_map = {}
        # Handle input/output nodes - map existing canvas nodes to pipeline node IDs
            ...
            if node.node_type == "input":
                ...
                for node_data in pipeline_data.get("nodes", []):
                    if node_data.get("node_type") == "output":
                        ...
                        node_map[node_data["id"]] = node
                        break
        # Create process and algorithm nodes
        for node_data in pipeline_data.get("nodes", []):
            if node_data.get("node_type") == "algorithm":
                ...
                for conn in pipeline_data_embedded.get("connections", []):
                    from_node_data = next((n for n in pipeline_data_embedded.get("nodes", []) if n['id'] == conn['from_node']), None)
    ...
    def _get_parameter_info(self, node_name):
        """Get parameter info for a node type"""
        for category in self.node_definitions.get('categories', []):
            for node in category.get('nodes', []):
                if node['name'] == node_name:
                    return node.get('parameters', [])
